export const ipRe = /^(\d{1,3}\.){3}\d{1,3}$/;

export const _TRUSTED = ['googleapis.com','google.com','gstatic.com','google-analytics.com','googletagmanager.com',
...
export const _registrable = (host) => {
...
export const _hostEndsWith = (a, b) => !(!a || !b) && (a === b || a.endsWith('.' + b) || b.endsWith('.' + a));

export const _normalizeText = (str) => {
  try {
    return (str || '').toString().normalize('NFD').replace(/[̀-ͯ]/g, '').toLowerCase();
  } catch (_) { return (str || '').toString().toLowerCase(); }
};
export const VN_SCAM_CONTENT_PATTERNS = [
...
export const _stickyState = { contentRich: false, sensitiveForm: false, formHijack: false, obfuscatedScript: false, keylogger: false, clipboardHijack: false, suspiciousExternalScript: false, downloadFile: false, networkUploadToExternal: false, hiddenIframe: false,
  rtcPeerConnection: false, windowOpenCount: 0, suspiciousPostMessage: false, dynamicScriptInjection: false, dynamicIframeInjection: false, sensitiveLSKeys: [], clipboardRead: false
};
...
export const detectBrandSurfaces = () => {
...
export const netState = { externalHosts: new Set(), uploadToExternal: false, externalPostHosts: new Set() };
export const permState = { requests: new Set() };
